...
This approach allows us to look at relationships between all variables at once. Note below that I am using all columns except `ID` because that graph would be a mess. You'll need to install the `GGally` package to run the following.
```{r}
# Load GGally package for ggpairs
library(GGally)

# Look at correlations between variables
GGally::ggpairs(turtles[ , 2:ncol(turtles)])
```
